...
const downloadM3u = async () => {
    try {
        ...
        fs.writeFileSync(`${process.env.DATA_FOLDER}/m3u-files/${date}.m3u`, m3uData.data);
...
const checkM3u = async () => {
    try {
        ...
        const currentM3uFiles = fs.readdirSync(`${process.env.DATA_FOLDER}/m3u-files`);
        ...
        if (difference >= 60) {
            oldM3uData = await parseM3u(fs.readFileSync(`${process.env.DATA_FOLDER}/m3u-files/${currentM3u}`, 'utf8'));
            fs.unlinkSync(`${process.env.DATA_FOLDER}/m3u-files/${currentM3u}`);
            currentM3u = await downloadM3u();
        }
...
const updateM3u = async () => {
    try {
        ...
        const currentM3uData = fs.readFileSync(`${process.env.DATA_FOLDER}/m3u-files/${currentM3u}`, 'utf8');
        if (currentM3uData) {
            parsedM3uData.set(await parseM3u(currentM3uData));
            const currentM3uUserParsed = await getUserParsedM3uData.fromImport({ ...parsedM3uData.get() });

            // If the old M3u file exists, compare it with the new M3u file
            if (oldM3uData) {
                const oldM3uUserParsed = await getUserParsedM3uData.fromImport(oldM3uData);
